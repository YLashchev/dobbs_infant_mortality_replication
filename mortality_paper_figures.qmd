...
df <- read_csv("data/dobbsbiannualbirthsdeaths_2024_09_16.csv")
aggregations <- read_csv("data/dobbsbiannualdeaths_aggregations.csv")
...
agg_deaths <- df %>% select(-starts_with("births")) %>% 
    ...
    select(time, category, deaths, state) %>%
...
relative_mortality_rate %>% filter(category == "total") %>% 
    ...
    geom_jitter(aes(x=time, y=relative_mr, col=state), height=0, alpha=0.5) +
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/relative_mortality.png", width=8, height=5)
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/relative_facet.png", width=14, height=10)
...
df %>% select(time, state, starts_with("deaths"), ban) %>% 
  ...
  tab_row_group(
   ...
   gtsave("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/missing_data.png", zoom=4)
...
df %>% select(time, state, starts_with("deaths"), ban) %>% 
  ...
  cols_align(
   ...
   gtsave("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/median_death_counts.png", zoom=4)
...
file_dir <- "~/Dropbox/abortion_results_and_data/posterior_samples/pois_deaths_rate_final"
suffix <- "biannual_rate_final"
...
for(i in 1:length(types)) {
    ...
    type_samples <- read_csv(sprintf("%s/Poisson_deaths_%s_%i_%s.csv", file_dir, type, model_rank, suffix))
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/ban_states_fit_plot.png", width=8, height=5)
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/ban_states_gap_plot.png", width=8, height=5)
```
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/race_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/congenital_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/neonatal_interval_plot.png", width=10, height=10)
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/total_interval_plot.png", width=10, height=10)


race_interval_agg / congenital_interval_agg / neonatal_interval_agg
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/agg_interval_plots.png", width=10, height=10)
...
make_mortality_table <- function(merged_df, 
                       ...
                       tab_caption = "Table 1. Estimated difference in cumulative observed vs expected infant deaths (count and rate) in all states that banned abortion in months affected by bans, overall, by race, timing of death, and cause of death.") {
...
#                 years=mean(interval(start_date, end_date) / years(1)))
  ...
  table_df %>%
    select(type, category, outcome, outcome_rate, death_counts_str, death_rate_abs_str, death_rate_pct_str) %>%
  ...
  tab_spanner(
    ...
    columns = c(outcome_rate, death_rate_abs_str, death_rate_pct_str)) |>
  tab_spanner(
    label = "Infant death count",
    columns = c(outcome, death_counts_str)) |>
  cols_label(
    outcome_rate = "Observed",
    death_rate_abs_str = html("Difference from expected <br>(95% CI)"),
    death_rate_pct_str = html("Percent change from expected<br>(95% CI)"),
    outcome = "Observed",
    death_counts_str = html("Difference from expected<br>(95% CI)"),
    category = ""
  ) |>
  tab_stub_indent(
    rows = category != "Total",
...
ftab <- make_mortality_table(all_samples, target_state="Ban States") |> 
        tab_footnote("Exposed months include January 2022 through December 2023 for Texas and January 2023 through December 2023 for other 13 states that banned abortion at 6 weeks or completely.")
ftab |> gtsave("~/Dropbox/abortion_results_and_data/mortality_figures/ban_states_table.png", zoom=4)
...
for(i in 1:length(treated_states)) {
  ...
  tab_caption = sprintf("Supplementary Table %i. Estimated difference in cumulative observed vs expected infant deaths (count and rate) in %s, from January 2023 through December 2023, overall and by socioeconomic characteristics", i, caption_string)
  
  make_mortality_table(all_samples, target_state=target_state, tab_caption=tab_caption) |> 
  tab_footnote("In the publicly available CDC Wonder data, cell counts between 1 and 9 are suppressed. Observed counts and rates therefore incorporate (mean) imputed infant deaths when observations are missing.") |>
  gtsave(filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/tables/%s_table.png", target_state), zoom=4)
  ...
  ggsave(filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/fit_and_gap_plots/%s_fit_plot.png", target_state), width=8, height=5)
  ...
  ggsave(sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/fit_and_gap_plots/%s_gap_plot.png", target_state), width=8, height=5)
...
for(t in types) {
  ...
  ggsave(rmse_res$rmse_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/ppc/%s_rmse_plot.png", t), width=10, height=figure_height)

  abs_res <- make_abs_res_ppc_plot(all_samples_ppc %>% filter(state %in% ppc_states, type == t), outcome="deaths")
  ggsave(abs_res$max_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/ppc/%s_abs_plot.png", t), width=10, height=figure_height)

  acf_ppc4 <- make_acf_ppc_plot(all_samples_ppc %>% filter(state %in% ppc_states, type == t),
                  lag=4, outcome="deaths") 
  ggsave(acf_ppc4$acf_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/ppc/%s_acf4_plot.png", t), width=10, height=figure_height)

  acf_ppc2 <- make_acf_ppc_plot(all_samples_ppc %>% filter(state %in% ppc_states, type == t),
                  lag=2, outcome="deaths") 
  ggsave(acf_ppc2$acf_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/ppc/%s_acf2_plot.png", t), width=10, height=figure_height)

  acf_ppc1 <- make_acf_ppc_plot(all_samples_ppc %>% filter(state %in% ppc_states, type == t), lag=1, outcome="deaths") 
  ggsave(acf_ppc1$acf_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/ppc/%s_acf1_plot.png", t), width=10, height=figure_height)

  uc_ppcs_obj <- make_unit_corr_ppc_plot(all_samples_ppc %>% filter(type == t), outcome="deaths")
  ggsave(uc_ppcs_obj$eval_plt, filename=sprintf("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/ppc/%s_unit_corr_plot.png", t), width=7, height=figure_height)
...
ggsave("~/Dropbox/abortion_results_and_data/mortality_figures/supplement_figures/driving_time.png")
